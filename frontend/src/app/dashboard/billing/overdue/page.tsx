...
export default function OverdueAccountsDetails() {
  ...
  const overdueAccounts = [
    { id: 'INV-2044', patient: 'James Robert', date: 'Oct 21, 2026', amount: '₹3,400.00', status: 'Overdue', daysOverdue: 15 },
    { id: 'INV-2049', patient: 'Susan Miller', date: 'Oct 15, 2026', amount: '₹1,500.00', status: 'Overdue', daysOverdue: 30 },
    { id: 'INV-2050', patient: 'Robert Taylor', date: 'Oct 10, 2026', amount: '₹5,600.00', status: 'Overdue', daysOverdue: 45 },
  ];
    ...
    <div style={{ minHeight: '100vh', width: '100vw', background: 'var(--bg-color)', padding: '3rem 4rem', transition: 'background 0.3s ease' }}>
      ...
      <header style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4rem' }}>
        <div>
          ...
          <h1 style={{ fontSize: '2.5rem', fontWeight: 800, color: 'var(--text-main)', marginBottom: '0.5rem', letterSpacing: '-1px' }}>Overdue Accounts</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem' }}>Accounts requiring immediate action for collection.</p>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead style={{ background: 'rgba(0, 0, 0, 0.05)', borderBottom: '1px solid var(--glass-border)' }}>
            <tr>
              ...
              <th style={{ padding: '1.2rem 1.5rem', color: 'var(--text-muted)', fontSize: '0.95rem' }}>Days Overdue</th>
          ...
          <tbody>
            {overdueAccounts.map((inv, idx) => (
              <tr key={idx} style={{ borderBottom: '1px solid var(--glass-border)', transition: 'background 0.2s', cursor: 'pointer' }} onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(0,0,0,0.02)'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
                ...
                <td style={{ padding: '1.2rem 1.5rem', color: '#e74c3c' }}>{inv.daysOverdue} Days</td>
                <td style={{ padding: '1.2rem 1.5rem', color: 'var(--text-main)', fontWeight: 700 }}>{inv.amount}</td>
                <td style={{ padding: '1.2rem 1.5rem' }}>
                  <button className="btn btn-primary" style={{ padding: '0.4rem 0.8rem', borderRadius: '8px', fontSize: '0.8rem', background: '#e74c3c' }}>Escalate Case</button>
                </td>
              </tr>
            ))}
          </tbody>
